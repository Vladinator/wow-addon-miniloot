...
local DefaultOptions = {
    ...
    ShortenPlayerNames = true, --- (TODO) enable to remove the realm name
    ShortenFactionNames = true, --- (TODO) enable to reduce the length of faction names
    ShortenFactionNamesLength = 10, --- (TODO) specify the length before reduction activates
    IconTrim = 8, --- (TODO) the px we want to trim from the texture icons
    IconSize = 0, --- (TODO) the px size we want the texture icon to be shown at
    ItemCount = false, --- (TODO) enable to add item count behind icons
    ItemCountBank = true, --- (TODO) enable to also include items in the bank
    ItemCountUses = true, --- (TODO) enable to count uses/charges as "one item"
    ItemCountReagentBank = true, --- (TODO) enable to also include items in the reagent bank
    ItemLevel = true, --- (TODO) enable to add item level behind icons
    ItemLevelEquipmentOnly = true, --- (TODO) enable to only show item level on equippable items
    ItemTier = true, --- (TODO) enable to add quality tier behind icons (DF crafting tier system)
